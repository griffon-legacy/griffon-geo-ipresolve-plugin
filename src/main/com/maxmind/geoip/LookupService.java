...
public class LookupService {
	...
	public Location getLocationwithdnsservice(String str) {
		...
		while (st.hasMoreTokens()) {
			...
			if (key.equals("co")) {
				...
				record.setCountryCode(value);
				record.setCountryName(countryName[i.intValue()]);
			}
			if (key.equals("ci")) {
				record.setCity(value);
			}
			if (key.equals("re")) {
				record.setRegion(value);
			}
			if (key.equals("zi")) {
				record.setPostalCode(value);
			}
			...
			if (key.equals("la")) {
				try {
					record.setLatitude(Float.parseFloat(value));
			...
			if (key.equals("lo")) {
				try {
					record.setLongitude(Float.parseFloat(value));
				} catch (NumberFormatException e) {
					record.setLatitude(0);
				}
			}
			...
			if (key.equals("dm") || key.equals("me")) {
				try {
					record.setMetroCode(Integer.parseInt(value));
					record.setDmaCode(Integer.parseInt(value));
				} catch (NumberFormatException e) {
					record.setMetroCode(0);
					record.setDmaCode(0);
				}
			}
			if (key.equals("ac")) {
				try {
					record.setAreaCode(Integer.parseInt(value));
				} catch (NumberFormatException e) {
					record.setAreaCode(0);
				}
			}
		}
		return record;
	...
	public synchronized Location getLocation(long ipnum) {
		...
		try {
			...
			// get country
			record
					.setCountryCode(countryCode[unsignedByteToInt(record_buf[0])]);
			record
					.setCountryName(countryName[unsignedByteToInt(record_buf[0])]);
			record_buf_offset++;
			...
			while (record_buf[record_buf_offset + str_length] != '\0')
				str_length++;
			if (str_length > 0) {
				record.setRegion(new String(record_buf, record_buf_offset,
						str_length));
			}
			...
			while (record_buf[record_buf_offset + str_length] != '\0')
				str_length++;
			if (str_length > 0) {
				record.setCity(new String(record_buf, record_buf_offset,
			...
			while (record_buf[record_buf_offset + str_length] != '\0')
				str_length++;
			if (str_length > 0) {
				record.setPostalCode(new String(record_buf, record_buf_offset,
						str_length));
			}
			record_buf_offset += str_length + 1;
			...
			record.setLatitude((float) latitude / 10000 - 180);
			record_buf_offset += 3;
			...
			for (j = 0; j < 3; j++)
				longitude += (unsignedByteToInt(record_buf[record_buf_offset
						+ j]) << (j * 8));
			record.setLongitude((float) longitude / 10000 - 180);
			...
			if (databaseType == DatabaseInfo.CITY_EDITION_REV1) {
				...
				if ("US".equals(record.getCountryCode())) {
					record_buf_offset += 3;
					for (j = 0; j < 3; j++)
						metroarea_combo += (unsignedByteToInt(record_buf[record_buf_offset
								+ j]) << (j * 8));
					record.setMetroCode(metroarea_combo / 1000);
					record.setDmaCode(metroarea_combo / 1000);
					record.setAreaCode(metroarea_combo % 1000);
				}
			}
		} catch (IOException e) {
